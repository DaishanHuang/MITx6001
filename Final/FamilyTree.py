...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ancestors_for_a = [node_a]
        ancestors_for_b = [node_b]

        while node_a != self.root:
            node_a = node_a.get_parent()
            ancestors_for_a.append(node_a)

        while node_b not in ancestors_for_a:
            node_b = node_b.get_parent()
            ancestors_for_b.append(node_b)

        ancestors_for_a = ancestors_for_a[:ancestors_for_a.index(node_b) + 1]

        lenA = len(ancestors_for_a)
        lenB = len(ancestors_for_b)
